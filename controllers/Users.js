...
export const getUsers = async (req, res) => {
    try {
        const response = await User.findAll({
            where: {
                role: ["customer", "vendor", "cooking_vendor"]
            },
...
export const createUser = async (req, res) => {
    ...
    try {
        ...
        if (role === "cooking_vendor") {
            const cookingType = await VendorType.findOne({ where: { code: "cooking_vendor" } });
            if (cookingType) {
                await Vendor.create({
                    userId: createUsers.id,
                    vendorTypeId: cookingType.id,
                    name: name,
                    isActive: 1
                });
            }
